...
Furthermore, the current IVOA definition of UCDs, UCD1+ (http://www.ivoa.net/documents/latest/UCD.html), defines the syntax as 'words' separated by semicolons, where the words consist of 'atoms' separated by periods.  The UCD for a Right Ascension column is therefore usually `pos.eq.ra` or, for the primary column in a table with multiple RAs, `pos.eq.ra;meta.main`.  But the definition of a Simple Cone Search (http://www.ivoa.net/documents/latest/ConeSearch.html), which predates UCD1+, requires that the field used for the position be in the older "POS_EQ_RA_MAIN" format.
